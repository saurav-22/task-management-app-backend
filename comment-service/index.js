...
app.post('/api/comments', authenticate, async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      ...
      [content, taskId, req.userId]
    );
    const [users] = await pool.query('SELECT email FROM users WHERE id = ?', [req.userId]);
    res.json({ id: result.insertId, content, taskId, userEmail: users[0].email });
  } catch (err) {
...
app.get('/api/comments', authenticate, async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      'SELECT c.*, u.email AS userEmail FROM comments c JOIN users u ON c.userId = u.id WHERE c.taskId = ?',
      [taskId]
    );
    res.json(rows);
  } catch (err) {
...
app.listen(3000, () => console.log('Comment Service running on port 3000'));